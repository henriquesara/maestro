...
import type { WorktreeProvenanceIncidentStore } from '../application/worktree-provenance-incident-store'
import type {
  WorktreeProvenanceIncidentKind,
  WorktreeProvenanceIncidentRecord
...
// worktree_provenance_incident (§7.3, B1) — the S3-ONLY incident channel,
// never settlement_incident. UNIQUE (correlation_id, kind, evidence_digest);
// `INSERT ... ON CONFLICT DO NOTHING`. No sweep clears `blocked`; there is no
// auto-resolution path in S3 — only the manual-resolution contract.
...
function toRecord(row: Row): WorktreeProvenanceIncidentRecord {
  return {
    id: row.id,
    correlationId: row.correlation_id,
    orcaDispatchId: row.orca_dispatch_id,
    sliceRef: row.slice_ref,
    kind: row.kind as WorktreeProvenanceIncidentKind,
...
export class SqliteWorktreeProvenanceIncidentStore implements WorktreeProvenanceIncidentStore {
  constructor(private readonly db: SyncDatabase) {}

  /** Defensive re-ensure, symmetric with SqliteWorktreeProvenanceStore (§7.5). */
  ...
  listBySlice(sliceRef: string): WorktreeProvenanceIncidentRecord[] {
    ...
    return (
      this.db
        .prepare(
          'SELECT * FROM worktree_provenance_incident WHERE slice_ref = ? ORDER BY raised_at, id'
        )
        .all(sliceRef) as Row[]
    ).map(toRecord)
  }
  ...
  hasOpenIncident(correlationId: string): boolean {
    ...
    const row = this.db
      .prepare(
        'SELECT 1 FROM worktree_provenance_incident WHERE correlation_id = ? AND resolved_at IS NULL LIMIT 1'
      )
      .get(correlationId)
  ...
  resolve(id: string, input: { resolvedAt: string; resolutionNote: string }): void {
    ...
    this.db
      .prepare(
        'UPDATE worktree_provenance_incident SET resolved_at = ?, resolution_note = ?, blocked = 0 WHERE id = ?'
      )
